...
`;class Header extends HTMLElement{constructor(){super()}connectedCallback(){this.innerHTML=svgIcons+headerHTML;const e=this.querySelector("#theme-toggle");e&&(e.checked=localStorage.getItem("color-theme")==="dark"||!("color-theme"in localStorage)&&window.matchMedia("(prefers-color-scheme: dark)").matches,e.addEventListener("change",()=>{e.checked?(document.documentElement.classList.add("dark"),localStorage.setItem("color-theme","dark")):(document.documentElement.classList.remove("dark"),localStorage.setItem("color-theme","light"))}));const t=this.querySelector('a[href=""]');t&&t.addEventListener("click",e=>{e.preventDefault();const t=document.getElementById("main-tools");t&&t.scrollIntoView({behavior:"smooth"})});const o=this.querySelector('#mobile-menu a[href=""]');o&&o.addEventListener("click",e=>{e.preventDefault();const t=document.getElementById("main-tools");t&&t.scrollIntoView({behavior:"smooth"})})}}customElements.define("header-component",Header);