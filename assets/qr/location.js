document.addEventListener('DOMContentLoaded', () => {
    ...
    const addressInputSection = document.getElementById('address-input');
    ...
    errorMessage.className = 'error-message text-red-600 dark:text-red-400 mt-2';
    errorMessage.id = 'error-message';
    errorMessage.setAttribute('aria-live', 'polite');
    const inputSection = document.querySelector('.input-section');
    inputSection.insertBefore(errorMessage, inputSection.children[2]);
    ...
    let locationType = 'coordinates'; // 'coordinates' or 'address'
    ...
    const generateQRCode = () => {
        ...
        if (locationType === 'coordinates') {
            ...
            if (!lat || !lon) {
                showError('Latitude and Longitude are required.');
                return;
            }
            const label = labelInput.value.trim();
            locationData = label ? `geo:${lat},${lon}?q=${encodeURIComponent(lat + "," + lon + "(" + label + ")")}` : `geo:${lat},${lon}`;
        } else {
            ...
            locationData = `geo:?q=${encodeURIComponent(address)}`;
        }
    ...
    const initiateDownload = (format) => {
        ...
        try {
            ...
            new QRCode(tempContainer, { text: locationData, width: highRes, height: highRes, colorDark: fgColor, colorLight: bgColor, correctLevel: QRCode.CorrectLevel.H });
    ...
    const switchInputMode = (mode) => {
        locationType = mode;
        const isCoordinates = mode === 'coordinates';

        coordinatesInput.style.display = isCoordinates ? 'block' : 'none';
        addressInputSection.style.display = isCoordinates ? 'none' : 'block';

        coordinatesBtn.classList.toggle('bg-indigo-600', isCoordinates);
        coordinatesBtn.classList.toggle('text-white', isCoordinates);
        coordinatesBtn.classList.toggle('bg-slate-200', !isCoordinates);
        coordinatesBtn.classList.toggle('text-slate-800', !isCoordinates);

        addressBtn.classList.toggle('bg-indigo-600', !isCoordinates);
        addressBtn.classList.toggle('text-white', !isCoordinates);
        addressBtn.classList.toggle('bg-slate-200', isCoordinates);
        addressBtn.classList.toggle('text-slate-800', isCoordinates);
    };
    ...
    coordinatesBtn.addEventListener('click', () => switchInputMode('coordinates'));
    addressBtn.addEventListener('click', () => switchInputMode('address'));
});