document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateQRCode = () => {
        ...
        if (!address) {
            showError('Bitcoin address is required.');
            return;
        }
        ...
        if (message) {
            uri += `${amount ? '&' : '?'}message=${encodeURIComponent(message)}`;
        }
    ...
    /**
     * Handles the download process, including on-demand loading of the PDF library.
    ...
    const downloadHandler = (format) => {
        ...
        // Check if jsPDF exists and load it if necessary for PDF download
    ...
    const initiateDownload = (format) => {
        const address = addressInput.value.trim();
        if (!address) {
            showError('Bitcoin address is required for download.');
            return;
        }

        // Reconstruct the URI to ensure it's up to date
        const amount = amountInput.value.trim();
        const message = messageInput.value.trim();
        let uri = `bitcoin:${address}`;
        if (amount) uri += `?amount=${amount}`;
        if (message) uri += `${amount ? '&' : '?'}message=${encodeURIComponent(message)}`;
        
        const fgColor = fgColorInput.value;
        const bgColor = bgColorInput.value;
        const logoSizePercent = parseInt(logoSizeSlider.value);
        const highRes = 1200; // High resolution for quality downloads
        ...
        try {
            // Generate high-res QR code in the temporary container
            new QRCode(tempContainer, { text: uri, width: highRes, height: highRes, colorDark: fgColor, colorLight: bgColor, correctLevel: QRCode.CorrectLevel.H });

            // Use a timeout to allow the QR code library to render the canvas
            setTimeout(() => {
                ...
                if (format === 'png') {
                    ...
                    link.download = 'bitcoin-qr-code.png';
                    link.click();
                } else if (format === 'pdf') {
    ...
    /**
     * Debounces QR code regeneration on input to prevent lag.
     * Only runs if a QR code has already been generated once.
     */
    const debouncedGenerate = () => {
        clearTimeout(debounceTimeout);
        if (!qrCodeGenerated) return; 
        debounceTimeout = setTimeout(generateQRCode, 250); // A 250ms delay is good for responsiveness
    };
